...
function createUser(email, role, firstName, lastName, password, position) {
...
if (Meteor.users.find().count() === 0) {
  if (Meteor.settings.defaultAccounts) {
    ...
    Meteor.settings.defaultAccounts.map(({ email, password, role, firstName, lastName, position }) => createUser(email, role, firstName, lastName, password, position));
  } else {
